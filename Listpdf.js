...
//     try {
//       const response = await fetch(`http://52.78.100.137:3001/documents?email=${email.userEmail}`, {
//         method: 'GET'
//       });
//       const data = await response.json();
//     console.log(data, 'helloinaaaaaaaaaaaaam')
...
//     try {
//       const response = await fetch(`http://52.78.100.137:3001/documents?email=${email}`, { method: 'GET' })
...
import pdf from './assets/pdf.png';
import docWord from './assets/docWord.png';
// import defaultIcon from './assets/defaultIcon.png'; // Replace with your default icon
import { UserContext } from './App';
import { AntDesign } from '@expo/vector-icons';
import DateTimePicker from '@react-native-community/datetimepicker';
...
export default function Listpdf() {
  ...
  function renderListItem({ item }) {
    ...
    if (fileExtension === 'pdf') {
      ...
      // Add additional conditions if you have other file types
      iconSource = docWord; // Provide a default icon for other file types
    }
    ...
    return (
      <View>
        <Pressable
          onPress={() => {
            openModal(item.fileUrl);
          }}
          style={styles.listItem}
        >
          <Image source={iconSource} style={{ width: 60, height: 60, borderRadius: 30 }} />
          <View style={{ alignItems: 'flex-start', marginLeft: 10, justifyContent: 'space-evenly', flex: 1 }}>
            <Text style={{ fontWeight: 'bold' }}>{lastSection}</Text>
            <Text style={{ fontWeight: 'bold' }}>Teacher: {item.teacher}</Text>
            <Text style={{ fontWeight: 'bold' }}>{`${new Date(item.date).getDate()}-${new Date(
              item.date
            ).getMonth() + 1}-${new Date(item.date).getFullYear()}`}</Text>
            <Text>{item.position}</Text>
          </View>
        </Pressable>
      </View>
  ...
  return (
    <View style={styles.container}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-around' }}>
        ...
        >
          <Text style={{ color: 'white', fontWeight: 600 }}>Sort By Date</Text>
          {arrow ? <AntDesign name="up" size={24} color="white" /> : <AntDesign name="down" size={24} color="white" />}
        </Pressable>
        <Pressable
        ...
        >
          <Text style={{ color: 'white', textAlign: 'center', fontWeight: 600 }}>Filter By Date</Text>
          <AntDesign name="filter" size={24} color="white" />
        </Pressable>
      </View>
      <FlatList
      ...
      >
        <Pressable
          style={{ padding: 20, marginTop: 40, zIndex: 2, alignItems: 'flex-end' }}
          onPress={() => closeModal()}
        >
          <AntDesign name="closecircle" size={24} color="black" />
        </Pressable>
        {modalVisible && (
          <WebView
            ...
            renderLoading={() => (
              <ActivityIndicator style={{ flex: 1, zIndex: 3 }} size="large" color="#5c0931" />
            )}
          />
        )}
      </Modal>
